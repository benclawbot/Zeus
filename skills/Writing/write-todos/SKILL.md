---
name: write-todos
description: Write clear, actionable todos that workers can execute without losing architectural intent. Use when the user explicitly asks to "create todos", "write todos", "break into tasks", "plan todos", "make todos", or turn a plan into work items. Produces self-contained todos with unambiguous expected outcomes, concrete examples, and explicit constraints.
---
...
Before returning the todo list, verify:
...
Return the todos as Markdown work items unless the user explicitly names a tracker or file format.
