...
## Image Generation Source of Truth

The image generation scripts (`scripts/generate_gpt_image.py`, `scripts/edit_gpt_image.py`) source `OPENAI_API_KEY` from **`~/.claude/skills/planf3/.env`** (maintained automatically by `hermes-oauth-openai-codex`). The canonical .env path is loaded with `override=True` so the OAuth-maintained token always wins over a stale shell variable.

If image generation is invoked and `OPENAI_API_KEY` is missing or its JWT is expired:

1. Surface the unblock command in the chat reply: `hermes-oauth-codex login` (from `C:\Users\thoma\hermes-oauth-openai-codex`).
2. The OAuth module refreshes the token, writes it to `~/.hermes/auth.json` under provider `openai-codex-pkce`, and updates `~/.claude/skills/planf3/.env` automatically.
3. Re-run the image generation step — no shell restart required because the script re-reads the .env on every invocation.

Never ask the user to paste an API key when the OAuth module is available — point them at `hermes-oauth-codex login` instead.
...
- Always include this CSS rule in the plan's `<style>` block so embedded images fit inside their `<figure>` regardless of source size:
  ```css
  figure img { max-width: 100%; height: auto; display: block; margin: 0 auto; border-radius: var(--r-md); }
  ```
  Generate images at `--size 1024x640` for inline use (see `workflows/image-generation.md`); the CSS handles any remaining scale-down.
...
- **Anchor every image prompt in the section it illustrates.** Open the plan, read the section's heading and first paragraph, and write the prompt naming the specific entities, modules, files, and relationships from that section. A reader who only sees the image should be able to tell which section it belongs to. Generic geometric shapes ("boxes", "arrows", "funnel") with no specific entities produce template art, not plan illustrations — see `workflows/image-generation.md` for the prompt template, quality checklist, and anti-patterns.
