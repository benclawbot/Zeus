...
# Always also load the canonical planf3 .env, which is maintained by
# hermes-oauth-openai-codex (hermes-oauth-codex login) and is the source
# of truth for the OPENAI_API_KEY used by image generation. Use override=True
# because the OAuth-maintained file is authoritative — if the shell has a
# stale or empty OPENAI_API_KEY from an earlier export, the canonical one wins.
PLANF3_DOTENV = Path.home() / ".claude" / "skills" / "planf3" / ".env"
if PLANF3_DOTENV.exists():
    load_dotenv(PLANF3_DOTENV, override=True)
...
def read_token() -> str:
    ...
    if not api_key:
        raise EnvironmentError(
            "OPENAI_API_KEY is not set. Planf3 sources this from "
            "~/.claude/skills/planf3/.env, which is maintained by "
            "hermes-oauth-openai-codex. To populate it, run:\n"
            "    pip install --user C:\\Users\\thoma\\hermes-oauth-openai-codex\n"
            "    hermes-oauth-codex login\n"
            "That writes a fresh ChatGPT-Plus OAuth bearer into the canonical "
            ".env automatically. As a one-off, you can also set OPENAI_API_KEY "
