...
fn should_skip_dir(name: &str) -> bool {
    matches!(name, ".git" | "node_modules" | "target" | "dist" | "build" | "out" | ".next" | ".turbo" | "coverage" | ".cache" | ".venv" | "venv" | "__pycache__")
}

fn is_text_candidate(path: &Path) -> bool {
    matches!(path.extension().and_then(|e| e.to_str()).unwrap_or(""),
        "rs" | "ts" | "tsx" | "js" | "jsx" | "mjs" | "cjs" | "json" | "md" | "toml" | "css" | "html" | "py" | "go" | "java" | "kt" | "swift" | "cpp" | "c" | "h")
}

/// Parse a single file and append every recognized symbol declaration.
pub fn extract_symbols_from_file(root: &Path, path: &Path, hits: &mut Vec<SymbolHit>) -> Result<(), String> {
    let rel = path.strip_prefix(root).unwrap_or(path).to_string_lossy().replace('\\', "/");
    let raw = match fs::read_to_string(path) { Ok(v) => v, Err(_) => return Ok(()) };
...
struct ParsedSymbol { name: String, kind: SymbolKind }
...
fn parse_symbol_line(line: &str, ext: &str) -> Vec<ParsedSymbol> {
    ...
    if ext == "rs" {
        ...
        if let Some(rest) = strip_any(line, &["impl "]) {
            if let Some(name) = take_token_before(rest, "< {") {
                out.push(ParsedSymbol { name: name.to_string(), kind: SymbolKind::Module });
            }
        }
    ...
    if matches!(ext, "ts" | "tsx" | "js" | "jsx" | "mjs" | "cjs") {
        if let Some(rest) = strip_any(line, &["export function ", "export async function ", "function ", "async function "]) {
            out.push(symbol_after(rest, SymbolKind::Function, "<({"));
        }
    ...
    if ext == "py" {
        if let Some(rest) = strip_any(line, &["def ", "async def "]) {
            let kind = if rest.starts_with("test_") || rest.contains("test_") { SymbolKind::Test } else { SymbolKind::Function };
...
impl SymbolKind {
    fn TypeAlias() -> SymbolKind { SymbolKind::Interface }
}
...
fn symbol_after<'a>(line: &'a str, kind: SymbolKind, terminators: &str) -> ParsedSymbol {
    ...
    ParsedSymbol { name: token.to_string(), kind }
}
...
pub fn search_symbols(index: &[SymbolHit], query: &str, seen: &HashSet<String>, max_results: usize) -> Vec<CodeSearchHit> {
    let q = query.trim().to_lowercase();
    if q.is_empty() { return Vec::new(); }
    let mut scored: Vec<CodeSearchHit> = index.iter()
        ...
        .map(|s| {
            ...
            let mut score = 0u32;
            if sym == q { score += 60; }
            else if sym.contains(&q) { score += 30; }
            if snippet.contains(&q) { score += 10; }
            // Token overlap bonus.
            let q_tokens: HashSet<&str> = q.split(|c: char| !c.is_ascii_alphanumeric()).filter(|t| t.len() > 2).collect();
            let s_tokens: HashSet<&str> = sym.split(|c: char| !c.is_ascii_alphanumeric()).filter(|t| t.len() > 2).collect();
            ...
            if seen.contains(&s.path) { score = score.saturating_sub(2); }
            CodeSearchHit {
                path: s.path.clone(),
                line: s.line,
        ...
        .filter(|h| h.score > 0)
        .collect();
    scored.sort_by(|a, b| b.score.cmp(&a.score).then(a.path.cmp(&b.path)).then(a.line.cmp(&b.line)));
...
pub fn search_substring(index: &[SymbolHit], query: &str, seen: &HashSet<String>, max_results: usize) -> Vec<CodeSearchHit> {
    let q = query.trim().to_lowercase();
    if q.is_empty() { return Vec::new(); }
    let mut hits: Vec<CodeSearchHit> = index.iter()
        ...
        .collect();
    hits.sort_by(|a, b| b.score.cmp(&a.score).then(a.path.cmp(&b.path)).then(a.line.cmp(&b.line)));
    ...
    for entry in walk_source(root) {
        ...
        for imp in extract_imports(&raw, &rel) {
            let entry = nodes.entry(rel.clone()).or_insert_with(|| DependencyNode {
                path: rel.clone(),
                kind: if rel.starts_with("src/") { "module".to_string() } else { "file".to_string() },
                depends_on: Vec::new(),
            });
            if !entry.depends_on.contains(&imp) { entry.depends_on.push(imp); }
        }
    }
    Ok(nodes.into_values().collect())
}

fn parse_package_json(root: &Path, nodes: &mut BTreeMap<String, DependencyNode>) -> Result<(), String> {
    let raw = fs::read_to_string(root.join("package.json")).map_err(|e| format!("read package.json: {e}"))?;
    let value: serde_json::Value = serde_json::from_str(&raw).map_err(|e| format!("parse package.json: {e}"))?;
    let mut deps = BTreeSet::new();
    if let Some(obj) = value.get("dependencies").and_then(|v| v.as_object()) {
        for key in obj.keys() { deps.insert(key.clone()); }
    }
    ...
    nodes.insert("package.json".to_string(), DependencyNode {
...
fn parse_cargo_toml(root: &Path, nodes: &mut BTreeMap<String, DependencyNode>) -> Result<(), String> {
    let raw = fs::read_to_string(root.join("Cargo.toml")).map_err(|e| format!("read Cargo.toml: {e}"))?;
    let mut deps = BTreeSet::new();
    ...
    for line in raw.lines() {
        ...
        if let Some((name, _)) = trimmed.split_once('=') {
            ...
            if name.is_empty() || !name.chars().next().map(|c| c.is_ascii_alphabetic()).unwrap_or(false) { continue; }
            deps.insert(name.to_string());
        }
    }
    nodes.insert("Cargo.toml".to_string(), DependencyNode {
...
fn extract_imports(raw: &str, rel: &str) -> Vec<String> {
    ...
    for line in raw.lines() {
        ...
        if trimmed.starts_with("use ") {
            ...
            let path = rest.split(|c: char| c == '{' || c == ' ' || c == ':').next().unwrap_or("");
        ...
        if trimmed.starts_with("import ") || trimmed.starts_with("export ") && trimmed.contains(" from ") {
            if let Some(idx) = trimmed.rfind("from ") {
                let after = trimmed[idx + 5..].trim().trim_matches(|c: char| c == '\'' || c == '"' || c == ';');
                out.insert(after.to_string());
            } else {
                // `import 'side-effect-only'`
                let token = trimmed.split_whitespace().find_map(|w| w.strip_prefix('\'').and_then(|s| s.strip_suffix('\'')))
                    .or_else(|| trimmed.split_whitespace().find_map(|w| w.strip_prefix('"').and_then(|s| s.strip_suffix('"'))));
                if let Some(t) = token { out.insert(t.to_string()); }
            }
            continue;
        }
        // Python `import x`, `from x import y`.
        if trimmed.starts_with("import ") || trimmed.starts_with("from ") {
            let token = trimmed.split_whitespace().nth(1).unwrap_or("").trim_end_matches(',');
            if !token.is_empty() { out.insert(token.to_string()); }
            continue;
        }
        // Go `import "x"`.
        if trimmed.starts_with("import ") || trimmed.starts_with("\"") {
            let token = trimmed.trim_start_matches("import ").trim_matches('"').trim_matches('`');
            if !token.is_empty() && !token.contains(' ') { out.insert(token.to_string()); }
        }
        let _ = rel;
    }
...
fn walk_source_inner(root: &Path, dir: &Path, out: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        ...
        } else if path.is_file() && matches!(path.extension().and_then(|e| e.to_str()).unwrap_or(""),
...
pub fn suggest_tests(root: &Path, source_files: &[String]) -> TestImpact {
    ...
    for src in source_files {
        ...
        let ext = Path::new(src).extension().and_then(|s| s.to_str()).unwrap_or("");
    ...
    let reasoning = if matched.is_empty() {
        ...
        format!("Touched source files: {}. Mapped to co-located test files and tests/ directories.", matched.join(", "))
    };
    TestImpact {
...
/// ranked hits.
pub fn search(index: &[SymbolHit], query: &str, seen: &HashSet<String>, max_results: usize) -> Vec<CodeSearchHit> {
    let mut hits = search_symbols(index, query, seen, max_results);
    if hits.len() < max_results {
        let extra = search_substring(index, query, seen, max_results);
        let existing: HashSet<(String, usize)> = hits.iter().map(|h| (h.path.clone(), h.line)).collect();
        for hit in extra {
...
mod tests {
    ...
    fn temp_workspace() -> PathBuf {
        let dir = std::env::temp_dir().join(format!("zeus_ci_{}_{:?}", std::process::id(), std::thread::current().id()));
    ...
    fn extracts_ts_symbols_and_searches_them() {
        ...
        writeln!(f, "export function runAgentTask() {{}}\nexport class AgentRunResult {{}}\n").unwrap();
    ...
    fn suggests_co_located_tests() {
        ...
        fs::write(dir.join("src").join("foo.test.ts"), "test('x', () => {});\n").unwrap();
        let impact = suggest_tests(&dir, &["src/foo.ts".to_string()]);
        assert!(impact.suggested_test_files.iter().any(|p| p.contains("foo.test.ts")));
